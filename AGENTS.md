...
This app is **Fulfilled** (protocol: Proof of Fulfillment). Product doctrine,
naming, privacy, and golden-case rules live in `AGENTS.project.md`. Engine
rules live in `src/lib/fulfillment/AGENTS.md`. Follow them with the same
priority as this file. If a request would publish a person, assert a clinical
or academic outcome, mutate an issued receipt, or loosen a golden test,
refuse.
